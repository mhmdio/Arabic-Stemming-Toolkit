	...
	public static void bootstrapInitialisation()
	{
		...
		try {
			if (useContext)
			{
				...
				try{
					terrier_home = (String)envCtx.lookup("terrier");
				}catch(NamingException ne) {
					throw new RuntimeException("NamingException finding terrier variables from envCtx",ne);
				}
				try{
					terrier_etc = (String)envCtx.lookup("etc");
				} catch(NamingException ne) {
					throw new RuntimeException("NamingException finding terrier variables from envCtx",ne);
				}
				try{
					propertiesFile = (String)envCtx.lookup("terrier.properties");
				}catch(NamingException ne) {
					throw new RuntimeException("NamingException finding terrier variables from envCtx",ne);
				}
			...
			else
			{
				...
				terrier_home = System.getProperty("terrier.home", "D:"+FILE_SEPARATOR+"Dropbox"+FILE_SEPARATOR+"workspace"+FILE_SEPARATOR+"AHS"+FILE_SEPARATOR+"terrier");
